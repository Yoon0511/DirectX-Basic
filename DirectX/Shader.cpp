...
D3D12_DEPTH_STENCIL_DESC CShader::CreateDepthStenceilState()
{
...
void CShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12RootSignature* pd3dRootSignature)
{
	...
	d3dPipelineStateDesc.DepthStencilState = CreateDepthStenceilState();
...
{
	return CShader::CompileShaderFromFile((wchar_t*)L"Shaders.hlsl", "VSInstancing", "vs_5_1", ppd3dShaderBlob);
}

D3D12_SHADER_BYTECODE CInstancingShader::CreatePixelShader(ID3DBlob** ppd3dShaderBlob)
{
	return CShader::CompileShaderFromFile((wchar_t*)L"Shaders.hlsl", "PSInstancing", "ps_5_1", ppd3dShaderBlob);
}
